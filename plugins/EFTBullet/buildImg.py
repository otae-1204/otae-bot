...
def build_ammo_info(ammoInfo, ammoMoreInfo: AmmoMoreInfo, qqId) -> int:
        if ammoInfo != None:
            ...
            # 绘制子弹文字
            title = BuildImage(w=0, h=0, font="default.ttf", font_size=200, plain_text="子弹数据", is_alpha=True, font_color=(55, 55, 55))
            ...
            # 绘制子弹名称
            name = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="名称 " + ammoInfo.name, is_alpha=True, font_color=(55, 55, 55))
            bg.paste(name, (1330, 1860),alpha=True)

            # 绘制子弹口径
            caliber = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="口径 " + ammoInfo.caliber, is_alpha=True, font_color=(55, 55, 55))
            ...
            typeImg = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="类型 " + typeName, is_alpha=True, font_color=(55, 55, 55))
            bg.paste(typeImg, (1330, 2280),alpha=True)

            # 绘制子弹id
            id = BuildImage(w=0, h=0, font="default.ttf", font_size=100, plain_text="id: " + str(ammoInfo.id), is_alpha=True, font_color=(60, 60, 60))
            bg.paste(id, (600, 2470),alpha=True)
            

            damageNum = ammoInfo.damage if ammoInfo.projectileCount == 1 else str(ammoInfo.damage) + "x"  + str(ammoInfo.projectileCount)
            # 绘制子弹肉伤
            damage = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="子弹肉伤 " + str(damageNum), is_alpha=True, font_color=(55, 55, 55))
            bg.paste(damage, (470, 2700),alpha=True)

            # 绘制子弹穿甲
            penetrationPower = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="子弹穿甲 " + str(ammoInfo.penetrationPower), is_alpha=True, font_color=(55, 55, 55))
            bg.paste(penetrationPower, (470, 2900),alpha=True)

            # 绘制子弹最大堆叠
            stackMaxSize = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="最大堆叠 " + str(ammoInfo.stackMaxSize), is_alpha=True, font_color=(55, 55, 55))
            ...
            if ammoInfo.tracer:
                if ammoInfo.tracerColor == "red" or ammoInfo.tracerColor == "tracerRed":
                    ...
                    colorName = "黄"
            tracerColor = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text="曳光颜色 " + colorName, is_alpha=True, font_color=(55, 55, 55))
            bg.paste(tracerColor, (470, 3300),alpha=True)

            # 绘制小出血修正
            lightBleedModifier = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text= f"小出血修正 {int(ammoInfo.lightBleedModifier * 100)}%",
                                            is_alpha=True, font_color=(55, 55, 55))
            bg.paste(lightBleedModifier, (1450, 2700),alpha=True)

            # 绘制大出血修正
            heavyBleedModifier = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text= f"大出血修正 {int(ammoInfo.heavyBleedModifier * 100)}%",
                                            is_alpha=True, font_color=(55, 55, 55))
            bg.paste(heavyBleedModifier, (1450, 2900),alpha=True)

            # 绘制受伤掉耐
            staminaBurnPerDamage = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text= f"被击打掉耐 {int(ammoInfo.staminaBurnPerDamage * 100)}%",
                                              is_alpha=True, font_color=(55, 55, 55))
            bg.paste(staminaBurnPerDamage, (1450, 3100),alpha=True)

            # 绘制护甲损伤
            armorDamage = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text= f"对护甲损伤 {int(ammoInfo.armorDamage)}%",
                                     is_alpha=True, font_color=(55, 55, 55))
            bg.paste(armorDamage, (1450, 3300),alpha=True)

            # 绘制弹速
            initialSpeed = BuildImage(w=0, h=0, font="default.ttf", font_size=120, plain_text= f"弹速 {int(ammoInfo.initialSpeed)}m/s",
                                      is_alpha=True, font_color=(55, 55, 55))
            ...
            if ammoInfo.accuracyModifier > 0:
                ...
                aMColor = (255, 0, 0)
            # 绘制精度
            accuracyModifier = BuildImage(w=0, h=0, font="default.ttf", font_size=120, is_alpha=True, font_color=(55, 55, 55), 
                                          plain_text= f"精度")
            ...
            # 绘制小标题2
            title2 = BuildImage(w=0, h=0, font="default.ttf", font_size=200, plain_text="获取途径", is_alpha=True, font_color=(55, 55, 55))
            ...
            for i in range(0, len(ammoMoreInfo.buyFor)):
                ...
                imgText = f"traders/{ammoMoreInfo.buyFor[i].source.capitalize()}-{level}.png" if ammoMoreInfo.buyFor[i].source != "fleaMarket" else "traders/fleaMarket.png"
                # 绘制源头像
                ...
                buyForName = BuildImage(w=0, h=0, font="default.ttf", font_size=140, plain_text=f"商人名称 {tradersName}", is_alpha=True, font_color=(55, 55, 55))
                ...
                buyForRequirements = BuildImage(w=0, h=0, font="default.ttf", font_size=140, plain_text=text, is_alpha=True, font_color=(55, 55, 55))
            ...
            for i in range(0, len(ammoMoreInfo.craftsFor)):
                # 绘制所需工作台等级
                craftsForLevel = BuildImage(w=0, h=0, font="default.ttf", font_size=140, plain_text=f"Level {ammoMoreInfo.craftsFor[i].level}",is_alpha=True, font_color=(55, 55, 55))
                ...
                for j in range(0, len(ammoMoreInfo.craftsFor[i].requirements)):
                    ...
                    cow = int(j / 5)
                    # print(cow)
                    posY = 4150 + len(ammoMoreInfo.buyFor) * 820 + i * 1350 + cow * 600 if len(ammoMoreInfo.craftsFor[i].requirements) > 5 else 4450 + len(ammoMoreInfo.buyFor) * 820 + i * 1350
                    # print(posY)
                    ...
                    # 绘制所需材料数量
                    craftsForRequirementsCount = BuildImage(w=0, h=0, font="default.ttf", font_size=140, plain_text=f"x{ammoMoreInfo.craftsFor[i].requirements[j]['count']}",is_alpha=True, font_color=(55, 55, 55))
