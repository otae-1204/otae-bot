...
config = Config()
server_list_name = [item for sublist in config.server_list.values() for item in sublist]
...
async def _(event: Event):
    ...
    print(f"传入数据为:{command},{event.get_user_id()},onebot,{event.get_session_id()}")
    try:
        match command["command"]:
            ...
            case "全部预测线":
                ...
                event_id = re.findall(r"\d+", command["message"])[0] if re.findall(r"\d+", command["message"]) else None
                if command["message"].endswith(tuple(server_list_name)) or command["message"].startswith(tuple(server_list_name)):
                    # 取出msg中的数字
                    server_index = get_server_index(command["message"].replace(event_id,"")) if event_id else get_server_index(command["message"])
